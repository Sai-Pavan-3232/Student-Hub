...
async function seed() {
  console.log("Seeding mock storage...");
  ...
  // Create sample clubs
  const club1 = await storage.createClub({
  ...
  const club2 = await storage.createClub({
  ...
  await storage.createClub({ name: "Photography Club", description: "Explore your creative side! Weekly photo walks, editing workshops, and gallery exhibitions.", category: "Arts", membersCount: 89 });
  await storage.createClub({ name: "Debate Society", description: "Sharpen your public speaking skills and engage in thought-provoking discussions.", category: "Academic", membersCount: 112 });
  await storage.createClub({ name: "Volunteer Corps", description: "Make a difference in the community through various volunteer opportunities.", category: "Community Service", membersCount: 178 });
  await storage.createClub({ name: "Entrepreneurship Club", description: "Connect with like-minded students, learn from founders, and launch your own startup.", category: "Business", membersCount: 145 });

  // Create sample events
  await storage.createEvent({ title: "Career Fair 2024", description: "Meet with top companies and explore internship and job opportunities.", date: "15 Dec", time: "10:00 AM - 4:00 PM", location: "University Center", clubId: null, attendingCount: 342 });
  await storage.createEvent({ title: "Hackathon: Build for Good", description: "24-hour coding challenge to create solutions for social impact. All skill levels welcome!", date: "20 Dec", time: "6:00 PM", location: "Engineering Building", clubId: club1.id, attendingCount: 128 });
  await storage.createEvent({ title: "Mental Health Workshop", description: "Learn stress management techniques and self-care practices for finals season.", date: "18 Dec", time: "2:00 PM - 4:00 PM", location: "Wellness Center", clubId: club2.id, attendingCount: 67 });
  await storage.createEvent({ title: "Winter Art Exhibition", description: "Showcase of student artwork from the fall semester. Refreshments provided.", date: "22 Dec", time: "5:00 PM - 8:00 PM", location: "Arts Gallery", clubId: null, attendingCount: 89 });

  console.log("Mock storage seeded successfully!");
  process.exit(0);
}

seed().catch((err) => {
  console.error("Error seeding mock storage:", err);
  process.exit(1);
});
